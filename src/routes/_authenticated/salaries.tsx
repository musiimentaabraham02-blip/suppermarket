...
function SalariesPage() {
  ...
  async function add(e: React.FormEvent) {
    ...
    const targetSupermarketId = isAdmin
      ? salaries[0]?.supermarket_id || "admin-branch"
      : supermarketId;
  ...
  const pendingAmount = salaries
    ...
    .reduce((acc, curr) => acc + Number(curr.monthly_salary), 0);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Manage employee payroll and track payment statuses{" "}
          {isAdmin ? "across all branches" : "for your branch"}.
        </p>
      </div>
          ...
          <Card className="bg-white/95 border-pink-100 shadow-md shadow-pink-100/30 backdrop-blur-md">
            <CardHeader className="pb-4 border-b border-pink-50">
              ...
              <CardDescription className="text-slate-500">
                Register a new staff member and their salary.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <form onSubmit={add} className="space-y-4">
                <div className="space-y-1.5">
                  <Label className="text-slate-600 font-semibold text-xs uppercase tracking-wide">
                    Full Name
                  </Label>
                  <Input
                    required
                    ...
                    className="bg-slate-50 border-pink-200 focus-visible:ring-pink-500 h-10 rounded-xl"
                ...
                <Button
                  type="submit"
                  className="w-full bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white shadow-md shadow-pink-500/25 border-0 h-11 rounded-xl mt-2 transition-all hover:scale-[1.02]"
                >
              ...
              <Table>
                <TableHeader className="bg-pink-50/50">
                  <TableRow className="hover:bg-transparent border-pink-100">
                    <TableHead className="font-semibold text-slate-600 w-[200px]">
                      Employee
                    </TableHead>
                    {isAdmin && (
                  ...
                  {filteredSalaries.length === 0 ? (
                    ...
                    filteredSalaries.map((s) => {
                      ...
                      return (
                        <TableRow
                          key={s.id}
                          className="hover:bg-pink-50/30 border-pink-50 transition-colors"
                        >
                          <TableCell>
                          ...
                          <TableCell>
                            <Badge
                              ...
                              className={`capitalize ${isPaid ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-0" : "bg-amber-100 text-amber-700 hover:bg-amber-200 border-0"}`}
                            >
                              {isPaid ? (
...
function StatCard({
  ...
  return (
    <Card
      className={`backdrop-blur-md shadow-md transition-all hover:-translate-y-1 ${warning ? "bg-amber-50/80 border-amber-200 shadow-amber-100/40" : "bg-white/95 border-pink-100 shadow-pink-100/20"}`}
    >
      <CardContent className="p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p
            className={`text-2xl font-extrabold tracking-tight mt-0.5 ${warning ? "text-amber-700" : "text-slate-800"}`}
          >
            {value}
          </p>
        </div>
