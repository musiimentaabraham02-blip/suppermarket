...
function LoginPage() {
  ...
  useEffect(() => {
    ...
    setSupermarkets([{ id: "preset", name: "(I'll be assigned by a director)" }]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-50">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-40 animate-pulse bg-gradient-to-br from-pink-400 to-rose-300" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full blur-[150px] opacity-35 bg-gradient-to-br from-rose-200 to-pink-100" />
        <div className="absolute top-[20%] right-[20%] w-[30%] h-[30%] rounded-full blur-[100px] opacity-25 mix-blend-multiply bg-gradient-to-br from-pink-300 to-rose-200" />
        ...
        <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 relative overflow-hidden bg-gradient-to-b from-pink-500 via-pink-600 to-rose-700 text-white shadow-xl shadow-pink-100/40 border-0">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <p className="text-lg text-white/80 font-medium leading-relaxed max-w-md">
              Sales, stock, salaries, expenses and analytics — across every branch, in one secure
              cloud platform.
            </p>
            <div className="flex gap-4 pt-4">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-full border-2 border-pink-600 bg-white/20 backdrop-blur-sm flex items-center justify-center text-xs font-semibold text-white"
                  >
                    U{i}
                  </div>
                ))}
                ...
                >
                  <form onSubmit={doSignup} className="space-y-4">
                        ...
                        <SelectContent className="bg-white border-pink-100 text-slate-800">
                          {supermarkets.map((s) => (
                            <SelectItem
                              key={s.id}
                              value={s.id}
                              className="hover:bg-pink-50 focus:bg-pink-50 focus:text-pink-700 text-slate-700"
                            >
                              {s.name}
                            </SelectItem>
                          ))}
                    ...
                    <p className="text-[11px] text-center text-slate-400 pt-2 font-medium">
                      ...
                      <strong className="text-slate-600 font-semibold">Manager</strong>. A Director
                      must promote you to full privileges.
                    </p>
                  </form>
                </TabsContent>
              </div>
