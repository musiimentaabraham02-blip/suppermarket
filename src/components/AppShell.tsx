...
export function AppShell({ children }: { children: ReactNode }) {
  const { roles, fullName, user, supermarketId, logout } = useAuth();
    ...
    try {
      ...
      if (error) {
        const isPermissionDenied =
          ...
          error.message?.toLowerCase().includes("has_role");
        if (isPermissionDenied) {
          toast.warning(
            "Local Setup Completed: We detected a database RLS permissions lockout (requires SQL grant). Your branch setup has been saved locally so you can continue using the dashboard!",
            {
              duration: 12000,
            },
          );
            ...
            <>
              <div className="px-3 pt-8 pb-2 text-[10px] font-bold text-white/60 uppercase tracking-widest flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Store className="size-3" /> Director Panel
                </div>
              </div>
              <div className="space-y-1">
                {NAV.filter((n) => !(n.roles as readonly string[]).includes("manager")).map(
                  (item) => {
                    const active = location.pathname === item.to;
                    const Icon = item.icon;
                    return (
        ...
        <div className="p-4 border-t border-white/10 bg-white/5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-white/60 capitalize font-medium tracking-wide">
                {roles[0] === "admin" ? "Director" : "Manager"}
              </div>
            </div>
            <Button
              variant="ghost"
              size="icon"
              onClick={logout}
              className="text-white/60 hover:bg-white/10 hover:text-white rounded-xl"
            >
        ...
        <header className="h-16 border-b border-pink-100 bg-white/60 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              size="sm"
              onClick={logout}
              className="text-slate-500 hover:bg-rose-50 hover:text-rose-600 rounded-xl font-semibold transition-colors"
            >
            ...
            <Button
              variant="ghost"
              size="icon"
              className="text-slate-600 hover:bg-pink-50 hover:text-pink-600 rounded-xl relative"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-pink-950/20 backdrop-blur-xl overflow-y-auto animate-fade-in">
          ...
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            ...
            <div
              ...
              style={{
                background: "linear-gradient(135deg, #ffc0cb, #ffd1d1)",
          ...
          <div className="relative z-10 w-full max-w-xl bg-white/95 border border-pink-100/80 rounded-3xl p-8 md:p-10 shadow-2xl backdrop-blur-2xl text-slate-800 animate-scale-up">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-slate-500 mt-3 max-w-md mx-auto">
                Welcome,{" "}
                <span className="font-semibold text-pink-600">{fullName || user?.email}</span>!
                Please select the supermarket branch you work in. This choice will configure your
                ERP workspace.
              </p>
            </div>
